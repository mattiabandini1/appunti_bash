...
| **FD Mapping**     | `<cmd> N>&M`                              | Reindirizza il **file descriptor N** per puntare allo stesso target del file descriptor M. (Esempio: `ls 2>&1 > file.txt` reindirizza stderr (`2`) verso stdout (`1`), che a sua volta è reindirizzato verso `file.txt`). | 158                 |
