...
| **Contenuti File**     | `cut -d "<delimitatore>" <file>`       | Serve per **usare il carattere `<delimitatore>` per dividere in colonne il file.** Può essere usato in combinazione al flag `-f` per estrarre una determinata colonna dopo che il file è stato suddiviso.      | comando `man cut`                         |
| **Contenuti File**     | `cut -f <num_colonna> <file>`          | Serve per **estrarre la colonna `<num_colonna>`** da una riga di un determinato file.                                                                                                                          | comando `man cut`                         |
| **Contenuti File**     | `read -n <num> <var>`                  | Legge **fino a `<num>` caratteri**, si ferma se incontra `\n` prima. La variabile `<var>` può diventare un'intera riga di un file in un ciclo `while-read`.                                                    |                                           |
| **Contenuti File**     | `read -N <num> <var>`                  | Legge **esattamente `<num>` caratteri**.                                                                                                                                                                       |                                           |
| **Text Processing**    | `sed` 's/`<trova>/<sostituisci>`/g'    | Trova qualsiasi occorrenza del carattere `<trova>` **e lo sostituisce con il carattere** `<sostituisci>` (può essere usato anche per cancellare determinati caratteri omettendo il carattere `<sostituisci>`). | comando `man sed` + il nostro eroe Gemini |
...
| **Info/Utility**       | `du -hs <directory>`                   | Mostra **l'occupazione del disco da parte della directory**. (flag `-h` serve per rendere il tutto più leggibile, flag `-s` mostra solo il totale della `<directory>`).                                        |                                           |
...
| **Variabili**            | `${!VAR}`                        | **Indirezione:** usa il valore di `VAR` come nome della variabile da leggere.                                                                                                  |             |
...
| **Condizionali**         | `test <condizione>`              | Valuta l'espressione e **restituisce un exit code** (0 se vero, 1 se falso). Viene usato con i flag `!` (negazione), `-a` (and) e `-o` (or).                                   | 121         |
| **Condizionali**         | `[ <condizione> ]`               | In molti sistemi non è altro che un **link al comando `test` vero e proprio**. ***N.B:*** *all'interno delle parentesi quadre le variabili vanno sempre inserite dentro `" "`* | 121         |
...
| Tipo               | Sintassi                                  | Descrizione                                                                                                                                                                                                               | Riferimenti         |
| :----------------- | :---------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :------------------ |
| **Output**         | `<cmd> > <file>`                          | **Reindirizza lo standard output** (`fd 1`) su un file, **sovrascrivendolo**.                                                                                                                                             | 150, 151            |
| **Output**         | `<cmd> >> <file>`                         | Reindirizza lo standard output (`fd 1`) su un file, **aggiungendolo in coda**.                                                                                                                                            | 150, 151            |
| **Input**          | `<cmd> < <file>`                          | **Reindirizza lo standard input** (`fd 0`) prendendolo dal file.                                                                                                                                                          | 150                 |
| **Error**          | `<cmd> 2> <file>`                         | Reindirizza lo standard error (`fd 2`) sul file.                                                                                                                                                                          | 153                 |
| **All Output**     | `<cmd> &> <file>`                         | Reindirizza **stdout e stderr** sullo stesso file.                                                                                                                                                                        | 154                 |
| **FD Mapping**     | `N>&M > file`                             | Reindirizza il **file descriptor N** per puntare allo stesso target del file descriptor M. (Esempio: `ls 2>&1 > file.txt` reindirizza stderr (`2`) verso stdout (`1`), che a sua volta è reindirizzato verso `file.txt`). | 158                 |
| **Pipe**           | `<cmd1>`\|`<cmd2>`                        | Collega lo standard output di `cmd1` allo standard input di `cmd2` (i processi eseguono in contemporanea in subshell).                                                                                                    | 156                 |
| **Pipe All**       | `<cmd1>`\|&`<cmd2>`                       | Collega **stdout e stderr** di `cmd1` allo standard input di `cmd2` (scorciatoia per `2>&1 \|`).                                                                                                                          | 166                 |
| **Apertura FD**    | `exec {FD}< <file>`                       | **Apre un file in lettura** e assegna il **file descriptor** libero ad una variabile (`FD`).                                                                                                                              | 145                 |
| **Apertura FD**    | `exec {FD}> <file>`                       | **Apre un file in scrittura**, eliminando il contenuto.                                                                                                                                                                   | file mitico di Samu |
| **Apertura FD**    | `exec {FD}>> <file>`                      | **Aggiunta in scrittura** in coda al contenuto del file.                                                                                                                                                                  | file mitico di Samu |
| **Apertura FD**    | `exec {FD}<> <file>`                      | **Apre un file in lettura e scrittura**.                                                                                                                                                                                  | file mitico di Samu |
| **Chiusura FD**    | `exec {FD}>&-`                            | **Chiude** lo stream associato al file descriptor nella variabile `FD`.                                                                                                                                                   | 148                 |
| **Command Sub**    | `OUT=`\`./file.exe\`<br>(importanti i ``) | Sostituisce il comando con l'output prodotto su stdout (sintassi **backticks**).                                                                                                                                          | 112                 |
| **Command Sub**    | `OUT=$(<cmd>)`                            | Sostituisce il comando con l'output prodotto su stdout (sintassi **tonde**).                                                                                                                                              | 112                 |
| **Raggruppamento** | `( <lista> )`                             | Esegue la lista di comandi in una **subshell** (copia dell'ambiente).                                                                                                                                                     | 101                 |
| **Raggruppamento** | `{ <lista> ; }`                           | Esegue la lista di comandi nella **shell corrente** (richiede `;` prima di `}`).                                                                                                                                          | 101                 |
| **Here Doc**       | `<cmd> << FINE ... FINE`                  | **Here Document**: ridirige l'input multi-linea specificato tra due delimitatori (`FINE`) allo standard input del comando.                                                                                                | 172, 173            |
...
| **Calcolo Digest SHA**                   | `sha256sum nomefile`                                                                  | **Calcola il _digest_ (hash) SHA-255** di un file, producendo una riga di output contenente il checksum e il nome del file.                                                                                                           |
| **Verifica Integrità File**              | `sha256sum -c nomefile.sha256`x480                                                    | **Verifica l'integrità** di un file confrontando il _digest_ ricalcolato con quello memorizzato nel file `nomefile.sha256`. Scrive `OK` se i digest combaciano o `FAILED` in caso contrario. Esiste anche il comando **`sha512sum`**. |
| **Calcolo DIgest con SSL (alternativa)** | `openssl dgst -sha256 nomefile`                                                       | Comando alternativo, fornito dal package `openssl`, per **calcolare il _digest_** del file specificato (in questo caso usando SHA-256).                                                                                               |
| **Elenco Chiavi Private**                | `gpg --list-secret-keys`                                                              | **Visualizza l'elenco delle chiavi private** (segrete) GPG già presenti nella home directory dell'utente (`/home/${USER}/.gnupg`).                                                                                                    |
| **Generazione Coppia Di Chiavi**         | `gpg --gen-key`                                                                       | **Genera la coppia di chiavi GPG** (pubblica e privata) RSA. Crea i file necessari, come `pubring.gpg`, `secring.gpg` e `gpg.conf`, nella directory di configurazione (`/home/${USER}/.gnupg`).                                       |
| **Creazione Firma GPG**                  | `gpg --armor --detach-sign nomefile`                                                  | **Crea una firma GPG _detached_** (separata) in formato **ASCII leggibile**. L'output è un file di testo chiamato `nomefile.asc`.                                                                                                     |
| **Creazione Firma GPG**                  | `gpg --detach-sign nomefile`                                                          | **Crea una firma GPG _detached_** (separata) in formato **binario**. L'output è un file binario chiamato `nomefile.sig`.                                                                                                              |
| **Creazione Firma GPG**                  | `gpg --clearsign nomefile`                                                            | **Crea una firma GPG _inline_**, generando un file di testo (`nomefile.asc`) che contiene sia il file originale che la firma, entrambi in formato ASCII leggibile.                                                                    |
| **Creazione Firma GPG**                  | `gpg --sign nomefile`                                                                 | **Crea una firma GPG _in-place_ (incorporata)**, generando un file binario (`nomefile.gpg`) che contiene sia il contenuto originale che la firma.                                                                                     |
| **Verifica Firma GPG**                   | `gpg --verify nomefile.asc nomefile`                                                  | **Verifica una firma GPG _detached_** (separata), confrontando il file di firma (`nomefile.asc`) con il file originale (`nomefile`).                                                                                                  |
| **Verifica Firma GPG**                   | `gpg --verify nomefile.gpg`                                                           | **Verifica una firma GPG _inline_ o _incorporata_**.                                                                                                                                                                                  |
| **Trovare ID Chiave Privata**            | `gpg --list-secret-keys --keyid-format long`                                          | **Trova l'ID completo (long format) della chiave privata** da utilizzare, specialmente quando sono presenti più chiavi installate.                                                                                                    |
| **Firma Di Un File**                     | `gpg --local-user ID_CHIAVE --detach-sign nomefile`                                   | **Firma un file specificando l'ID della chiave privata** (`ID_CHIAVE`) da utilizzare per la firma.                                                                                                                                    |
...
| `ssh <account>@<host> -o PubkeyAuthentication=no`<br>                              | Si connette a SSH **forzando l'uso della password** invece dell'autenticazione a chiave pubblica/privata.                                                                        |                                                                                                        |
...
| `ssh -T <host> /bin/bash <<FINESCRIPT ... FINESCRIPT`<br>```b<br>```               | Esegue una **sequenza di comandi** o uno script localmente definito sul server remoto.                                                                                           | La stringa `FINESCRIPT` (o qualsiasi altra stringa scelta) delimita l'inizio e la fine della sequenza. |
