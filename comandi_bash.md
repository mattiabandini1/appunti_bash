...
| **Calcolo Digest SHA**                   | `shasum256 nomefile`                                                                  | **Calcola il _digest_ (hash) SHA-255** di un file, producendo una riga di output contenente il checksum e il nome del file.                                                                                                           |
| **Verifica Integrità File**              | `sha256sum -c nomefile.sha256`                                                        | **Verifica l'integrità** di un file confrontando il _digest_ ricalcolato con quello memorizzato nel file `nomefile.sha256`. Scrive `OK` se i digest combaciano o `FAILED` in caso contrario. Esiste anche il comando **`sha512sum`**. |
